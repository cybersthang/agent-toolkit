...
| `gap_completeness_gate` | Stop | **BLOCK** done-claim while `.open_gaps.json` has entries `status: open` — chặn drip-feed anti-pattern (`feedback_exhaustive_analysis`). 3 resolution tiers: fix → re-emit · `gap-defer: G<N> <reason>` · `gap-cant-fix: G<N> <reason>` (escalate DEV). Whole-gate bypass: `bypass-gap-gate: <reason>` in prompt. Skips on autonomy active (v0.19.0). |
| `scope_completeness_gate` | Stop | **BLOCK** done/full-claim while `.scope_manifest.json` còn item `status: pending` — chặn partial-done trên multi-item request (R9, `feedback_exhaustive_analysis`). Manifest derive từ STRUCTURED artifact: tasks.md > acceptance_evals > TodoWrite≥3 (KHÔNG parse DEV prompt keyword). 3 resolution tiers: `scope-done: S<N>` · `scope-defer: S<N> <reason>` · `scope-cant: S<N> <reason>` (escalate DEV). Whole-gate bypass: `bypass-scope-gate: <reason>` in prompt. Silent khi không manifest (zero friction). Default `block` (v0.23.0). |
