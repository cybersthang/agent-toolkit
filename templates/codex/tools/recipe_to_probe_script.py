...
def _render_script(probe_id: str, recipe: str,
    ...
    return (
        ...
        f"PROBE_ID = \"{probe_id}\"\n"
        "# L2: BASE URL precedence — HOTPOT_BASE_URL > TOOLKIT_TEST_URL > localhost:8069.\n"
        "# The localhost fallback only works for the default Odoo dev setup;\n"
        "# CI / staging must export one of the env vars or the probe will\n"
        ...
        "BASE = os.environ.get(\"HOTPOT_BASE_URL\") or os.environ.get(\"TOOLKIT_TEST_URL\") or \"http://localhost:8069\"\n"
        "if BASE == \"http://localhost:8069\" and not os.environ.get(\"TOOLKIT_TEST_ALLOW_LOCALHOST\"):\n"
        "    sys.stderr.write(\"[probe] WARN: using default http://localhost:8069 — set HOTPOT_BASE_URL or TOOLKIT_TEST_ALLOW_LOCALHOST=1\\n\")\n"
        "LOGIN = os.environ.get(\"TOOLKIT_TEST_LOGIN\", \"admin\")\n"
        "PASSWORD = os.environ.get(\"TOOLKIT_TEST_PASSWORD\", \"admin\")\n"
        "DB = os.environ.get(\"HOTPOT_DB\") or os.environ.get(\"TOOLKIT_TEST_DB\", \"test\")\n"
        "ODOO_PID = int(os.environ.get(\"HOTPOT_ODOO_PID\", \"0\"))\n\n"
