---
name: clarify
description: Spec Kit Phase 2 — CLARIFY. The user invokes `/clarify` to be interviewed one-question-per-turn until every Open Question in the spec is closed. Cross-checks against `.agent-toolkit/decision-log.md` (ADR) + `.agent-toolkit/invariants.json` + `.agent-toolkit/constitution.md` + the in-code glossary. After DEV says "done", refines `acceptance_evals` and auto-fires `/tasks <slug>` (which STOPs for DEV review). Open this skill WHEN the user says "clarify", "grill me", "stress-test the spec", "challenge me", "/clarify".
---

# Clarify — Spec Kit Phase 2: CLARIFY

> Purpose: a spec produced in PLAN always has fuzzy spots. The CLARIFY phase
> is NOT for the agent to "present the plan for approval" — it is the inverse:
> the user is interviewed one question at a time until every decision is
> settled, and only then does TASKS (Phase 3) open.

## When to apply

- The user types `/clarify`.
- The user says "challenge me", "grill me", "stress-test the plan",
  "I want to be clarified".
- After `/plan`, the user replies "clarify" / "go clarify" / "đi tiếp".

## When to SKIP

- No spec yet at `.agent-toolkit/specs/**/<slug>.md` (run `/plan` first).
- The user says "just implement, skip clarify" — respect that, but warn
  that auto-chain `/implement` requires `acceptance_evals` to be defined,
  and that has to come from `/clarify`. The user can run `/eval-define`
  manually as an escape hatch.
- Read-only questions ("how does X work") — not a clarify; use
...
1. **Locate spec — branch-scoped**: `Glob: .agent-toolkit/specs/**/<slug>.md`.
   Pick most-recently-modified if multiple hits. If multiple drafts exist
   for different features → ask the user which one ("3 specs are in
   draft: X, Y, Z. Clarify which?").
2. **Load context** (parallel reads):
   - `.agent-toolkit/constitution.md` — toolkit principles; no Q may
     contradict.
...
3. **Print one line**: `Clarify mode ON — spec: <slug> · ADRs loaded · ready.`
...
When the clarify touches a decision shaped like:
...
A clarify question is *HIGH-stakes* when the outcome will:
...
When the clarify touches a decision like "classify X" / "identify Y" /
...
**Format inside the clarify question**:
...
> is BANNED in clarify answers and in proposed implementations.
...
the SEMANTIC MAPPING is correlational. Symptoms to catch in clarify:
...
- All other decisions → update the spec at the path Glob resolved — move
  the item from "Open Questions" to "Implementation Decisions". Bump
  `last_updated`.

### End of clarify

The user types "done" / "xong" / "đủ" → the agent runs FOUR steps in order:

**Step A — Auto-fold `/eval-define` (REQUIRED)**

Before printing the clarify report, the agent runs the `/eval-define`
workflow inline (without requiring the user to type the command):

1. For each decision settled in this clarify session, derive 1+
   acceptance_eval entry per `claim-falsification` recipe catalog
   (subject_X, property_P → recipe match).
2. Smoke-test at least one representative probe with a **30 s timeout +
   1 retry on transient failure** (MCP cold-start, network blip):
   - Probe FAIL with transient error pattern (`timeout`, `connection
     refused`, `mcp:cold-start`, `EPIPE`) → retry exactly once after a
     2 s wait. If retry passes, log "smoke OK (after 1 retry)" and proceed.
   - Probe FAIL with non-transient error (assertion mismatch, table
     missing, auth) → do NOT retry. Set `smoke: failed` + emit error
     row; do NOT auto-fire `/tasks`; tell DEV to fix probe first.
   - DEV may type `clarify: skip-smoke <eval-id>` to bypass smoke for
     ONE eval (probe documented but un-exercised). Bypass logs the
     reason in spec body and sets `smoke: skipped-by-dev`.
3. Append/refine the `acceptance_evals:` YAML block in spec frontmatter;
   set `eval_status: defined`.

**Step B — Print the clarify report**

```
Clarify DONE. Spec updated: <path-from-Glob>
  · Settled: <N> decisions
  · Promoted to ADR: <list ADR-NNN if any>
  · Promoted to invariant: <list inv-id if any>
  · Still TBD: <count of unresolved questions>
  · Refined acceptance_evals: <M> probes (<K> smoke-OK, <P> pending)
```

**Step C — Set spec frontmatter `status: clarified`** + `last_updated`.

**Step D — Auto-fire `/tasks <slug>`** inline (same turn):

The clarify skill calls `tasks-breakdown` skill to emit tasks.md next to
the spec. The `tasks-breakdown` skill prints a DEV review prompt and
STOPs — the user reviews tasks.md, edits if needed, then types
`/implement <slug>` to authorize the auto-chain.

**KEY**: clarify ends at tasks.md emit + STOP. Do NOT open the implement
phase in the same turn. The DEV review gate at tasks.md is intentional.

If the user wants to skip the auto-eval-define (rare — e.g. exploratory
spike, no testable claims), they say "skip evals" before "done" → Step A
is skipped, spec gets `eval_status: skipped-by-user`. `/tasks` then
refuses to auto-fire (no defined evals) — DEV must run `/tasks <slug>`
manually with `--no-evals` if they really want.

## Auto-discover during clarify
...
| "After clarify I'll just implement" | Wrong. Clarify ends with auto-fired `/tasks` + STOP. DEV reviews tasks.md, then types `/implement`. Skipping the gate = drift unprotected. |
...
- The agent opens Edit/Write/Bash to mutate code during a clarify turn.
...
- Clarify ends without auto-firing `/tasks` (skill broke the auto-chain).
- Clarify auto-fires `/implement` (must STOP after `/tasks` for DEV review).

## Sibling skills

- `plan-feature` — Phase 1, produces the spec used as clarify input.
- `clarification-gate` — runs at the prompt level; clarify runs at the
  decision level.
- `tasks-breakdown` — Phase 3, auto-fired after clarify completes.
- `analyze-artifacts` — Phase 3.5, runs from `/implement` to lint artifacts.
...
- `/adr-add` — promote a clarify decision to an ADR.
