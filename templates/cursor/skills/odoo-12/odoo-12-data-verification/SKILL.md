...
- Comparing aggregates with `==` in your head. => use `compare_with_expected` so the result is JSON-canonical.
- Running once and declaring "looks deterministic." => always use `consistency_check_eval` for that claim.
