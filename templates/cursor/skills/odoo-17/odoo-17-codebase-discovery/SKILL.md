---
name: odoo-17-codebase-discovery
description: Discover Odoo 17 modules, manifests, model inheritance and XML IDs through the codebase MCP without broad-reading files. Module-agnostic - works for every addon under the configured roots.
---

# Odoo 17 — Codebase Discovery (MCP-first)
...
1. **Narrow first.** When the workspace has multiple addon roots, pass `root_hint` (e.g. `custom_addons`).
2. **Discover then drill.** `discover_modules` -> pick one -> `read_manifest` -> `find_inheritance_chain`.
3. **Read by chunk.** `read_file_chunk` with start/end_line, never full files unless small.
