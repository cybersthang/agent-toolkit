...
description: Read and search JIRA tickets through the jira_production and jira_preproduction MCP servers. Use when the user mentions a ticket key, asks for sprint/standup context, or needs requirements pulled into the conversation. Works for any Odoo project version — JIRA workflow is independent of Odoo major version.
---

# Odoo — JIRA Workflow (version-agnostic)

Two JIRA profiles are wired up. **Pick the right one.** This skill body
is 100% version-agnostic — the same JIRA tools work whether the project
runs Odoo 12, 17, or any future version.
...
`{{ENV_PREFIX}}_JIRA_*` vars at launch. To see the URL the current
session resolved to, call `env_status` on the corresponding MCP server.
Never hard-code URLs or credentials anywhere else.

> If the project preset doesn't ship JIRA MCP (e.g. `odoo-12` generic
> or `odoo-17`), neither tool will be available. Check `.mcp.json`; add
> JIRA via `agent-toolkit.config.json` if needed.
