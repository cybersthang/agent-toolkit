...
| 13/14/15/16 | apply `odoo-17-tdd-pitfalls.md` + flag MEDIUM "v14-16 differ on `mail.thread` test setup (mail v1 era; v19 will refactor to v2); HttpCase URL routes were renamed in v15. Cross-check via `codebase.search_text` for the specific test helper on the target version before applying. Dedicated `odoo-<N>` rule pack ships for every v13-20 — consult those for stack defaults." |
