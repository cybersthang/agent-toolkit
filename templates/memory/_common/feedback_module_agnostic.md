...
type: feedback
---
Rules and skills under `.cursor/rules/` (alwaysApply) and `.cursor/skills/<name>/SKILL.md` must be **generic** — usable for any Odoo 12 addon under any addon root. Do not embed concrete module names like `<proj>_sale`, `oca_*`, or `web_*` into invariant rules. When a concrete module is needed, discover it at runtime via `codebase.discover_modules` / `module_dependencies` / `find_inheritance_chain`.
