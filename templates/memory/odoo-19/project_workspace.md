...
---
- Root: `{{WORKSPACE_ROOT}}`.
- Stack: Odoo 19 (Python {{STACK_LANGUAGE_VERSION}}, OWL v2 frontend, PostgreSQL >= 13).
- Database (default): `{{DEFAULT_DB}}`.
- Python: `{{PYTHON_BIN}}` (3.10+ required).
- Addon roots:
{{ADDON_ROOTS}}
...
**How to apply:**
- Pre-v13 conventions DO NOT apply: methods are recordset by default, no `@api.multi`.
